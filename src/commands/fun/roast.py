    ...
    def __build_roast_prompts(
        ...
        if is_supportive:
            style_instruction = (
                ...
                f"как лучший друг, который реально верит в него. Без сарказма, с душой. Строго одно предложение."
            )
        else:
            style_instruction = (
                f"Напиши жёсткий саркастический роаст на @{target_username} в стиле стендап-комика. "
                f"Строго одно предложение. Злой юмор, чёрный сарказм, смешно и больно. "
                f"Обязательно упомяни @{target_username} в тексте. "
                f"ЗАПРЕЩЕНО начинать с шаблонов вроде 'ты такой же...' или сравнений — будь оригинальным."
            )

        if history_text:
            system_prompt = (
                "Ты стендап-комик в группе друзей-геймеров. "
                "Пишешь роасты — строго одно предложение, никаких шаблонных сравнений вроде 'ты такой же...'. "
