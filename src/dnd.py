...
async def __generate_round(
    ...
    if mode == "pvp":
        system_prompt = (
            "Ты генератор сценариев для D&D PvP-игры в Telegram-чате геймеров.\n"
            "Твой ответ должен быть строго в формате (без лишних слов):\n"
            "СЦЕНАРИЙ: <2-3 предложения — смешная абсурдная завязка драки, кто с кем и за что>\n"
            "Д1: <боевой приём или трюк против соперников, 3-6 слов>\n"
            "Д2: <боевой приём или трюк против соперников, 3-6 слов>\n"
            "Д3: <боевой приём или трюк против соперников, 3-6 слов>\n\n"
            "Только русский язык. Никакого другого текста."
        )
    elif mode == "heist":
        system_prompt = (
            "Ты генератор сценариев для D&D-ограбления в Telegram-чате геймеров.\n"
            "Твой ответ должен быть строго в формате (без лишних слов):\n"
            "СЦЕНАРИЙ: <2-3 предложения — место ограбления, что охраняется, первое препятствие>\n"
            "Д1: <хитрый/стелс вариант действия, 3-6 слов>\n"
            "Д2: <хитрый/стелс вариант действия, 3-6 слов>\n"
            "Д3: <хитрый/стелс вариант действия, 3-6 слов>\n\n"
            "Только русский язык. Никакого другого текста."
        )
    else:
        system_prompt = (
