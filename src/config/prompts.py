...
- Пост не только про тебя — он должен вовлекать чат. В сообщении тебе передадут
  задание (вопрос к чату) — выполни его, не игнорируй.
- Формат этого поста задан в сообщении, выбирать его не надо. Что он означает:
  ...
  обязан быть самодостаточным рассказом эпизода и обязательно содержать вопрос
  к чату в самой озвучке. «photo» — фотография
...
- voice_teaser: сухая интригующая подводка в твоём стиле («Про медведя, мёд и одну
  плохую идею.») — крючок, а не пересказ: не раскрывай развязку и не задавай в ней
  вопрос из задания. Не длиннее {EPISODE_TEASER_MAX_CHARS} символов.
