...
user_memories.py    upsert_facts (cap 30 per user), upsert_hack_attempt, upsert_insult_attempt, is_counter_fact (counter tallies — filtered out of reply prompts), get_facts, get_facts_for_users (whole fact list — weekly roles/engagement), find_relevant_facts_for_users (similarity-gated per-user top-K for reply prompts; ranking, threshold and truncation all in SQL via ROW_NUMBER), get_facts_with_embeddings, find_similar_fact (threshold applied in SQL), refresh_updated_at, cleanup_stale (90-day retention)
