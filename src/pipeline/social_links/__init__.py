...
Instagram Reel, Reddit post, and long-form YouTube video links all follow
the same shape: regex-detect a link in chat, gate it (repost dedup + daily
cap, same TtlGate pattern as src.pipeline.shorts), fetch metadata + top
comments (no transcript/audio/vision analysis — that stays exclusive to
shorts.py, kept deliberately separate; see the design doc), and inject a
labelled text block into the pipeline. Instagram alone may also attach
downloaded video bytes for Telegram to repost.
...
SOCIAL_LINK_DESCRIPTION_CHAR_LIMIT = 2000      # caps YouTube description / Reddit selftext
...
class LinkHandler(Protocol):
    ...
    instance that the registry in this package's ``HANDLERS`` list (added
    once all three handlers exist — see Task 3) references directly.
    """
...
def render_comment_lines(comments: list[dict], max_comments: int, char_limit: int) -> str:
    ...
    dicts have the same ``text``/``like_count`` shape. reddit_post.py's
    comments come from a differently-shaped JSON payload
    (``author``/``body``/``score``) and render their own — not a fit here.
...
from src.pipeline.social_links import instagram_reel, reddit_post, youtube_video  # noqa: E402
